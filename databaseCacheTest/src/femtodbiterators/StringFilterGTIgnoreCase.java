...
public class StringFilterGTIgnoreCase implements FemtoDBIterator{
	private int 			column;
	private String 			compareValue;
	private FemtoDBIterator source;
	private RowAccessType	currentRow;
	
	StringFilterGTIgnoreCase(final int column, final String compareValue, final FemtoDBIterator source)
	{
		this.column 		= column;
	...
	public boolean hasNext() throws FemtoDBConcurrentModificationException,FemtoDBIOException {
		...
		while(true)
		{
			...
			if(temp.getString(columnL).compareToIgnoreCase(compareValueL) > 0)
			{
