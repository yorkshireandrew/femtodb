...
public class ByteFilterGT implements FemtoDBIterator{
	private int 			column;
	...
	ByteFilterGT(final int column, final byte compareValue, final FemtoDBIterator source, final boolean invert)
	{
		this.column 		= column;
	...
	public boolean hasNext() throws FemtoDBConcurrentModificationException,FemtoDBIOException {
		int columnL 			= column;
		byte compareValueL 		= compareValue;
		...
		while(true)
		{
			...
			if(temp.get_byte(columnL) < compareValueL)
			{
