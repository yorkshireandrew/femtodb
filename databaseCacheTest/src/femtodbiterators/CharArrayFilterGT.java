...
public class CharArrayFilterGT implements FemtoDBIterator{
	private int 			column;
	...
	CharArrayFilterGT(final int column, final char[] compareValue, final FemtoDBIterator source, final boolean invert)
	{
		this.column 		= column;
	...
	public boolean hasNext() throws FemtoDBConcurrentModificationException,FemtoDBIOException {
		...
		while(true)
		{
			...
			if(isGreaterThan(temp.get_charArray(columnL),compareValueL))
			{
	...
	final private boolean isGreaterThan(char[] a, char[] b)
	{
		int lena = a.length;
		int lenb = b.length;
		int minlen = lena;
