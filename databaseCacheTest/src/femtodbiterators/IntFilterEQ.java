...
public class IntFilterEQ implements FemtoDBIterator{
	private int 			column;
	...
	IntFilterEQ(final int column, final int compareValue, final FemtoDBIterator source, final boolean invert)
	{
		this.column 		= column;
	...
	public boolean hasNext() throws FemtoDBConcurrentModificationException,FemtoDBIOException {
		int columnL 			= column;
		int compareValueL 	= compareValue;
		...
		while(true)
		{
			...
			if(temp.get_int(columnL) == compareValueL)
			{
