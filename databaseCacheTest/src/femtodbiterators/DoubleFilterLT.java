...
public class DoubleFilterLT implements FemtoDBIterator{
	private int 			column;
	...
	DoubleFilterLT(final int column, final double compareValue, final FemtoDBIterator source, final boolean invert)
	{
		this.column 		= column;
	...
	public boolean hasNext() throws FemtoDBConcurrentModificationException,FemtoDBIOException {
		int columnL 			= column;
		double compareValueL 	= compareValue;
		...
		while(true)
		{
			...
			if(temp.get_double(columnL) < compareValueL)
			{
